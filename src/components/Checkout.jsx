...
const Checkout = (props) => {
  ...
  const priceToNumber = (numero) => {
    ...
    const number = Number(splited.join(''))
    return number
  }
  ...
  const funcCantidades = (cantidad) => {
    cantidadTotal += cantidad
    return cantidad
  }

  const subtotal = (precio, cantidad) => {
    const sumatoria = priceToNumber(precio) * cantidad
    total += sumatoria
    return sumatoria
  }

  const Totales = (precio, cantidad) => {
    const subTotal = priceToNumber(precio) * cantidad
          ...
          <div style={{ textAlign: 'center' }} className="cardcheck cardCheckout">
            ...
            <ul style={{ maxWidth: '100%' }} className="list-group list-group-flush">
              <li className="list-group-item ">
                ...
                {' '}
                <input style={{ textAlign: 'center' }} onChange={handleChange} defaultValue={`${props.usuario.nombre} ${props.usuario.apellido}`} type="text" name="nombreyapellido" className="inputDetail" />
              </li>
              <li className="list-group-item">
                <p className="pCheckout">Email:</p>
                {' '}
                <input style={{ textAlign: 'center' }} onChange={handleChange} defaultValue={props.usuario.email} type="email" name="email" className="inputDetail" />
              </li>
              <li className="list-group-item">
                <p className="pCheckout">Dirección de envío:</p>
                {' '}
                <input style={{ textAlign: 'center' }} onChange={handleChange} defaultValue={props.usuario.domicilio ? props.usuario.domicilio : ''} type="text" name="direccion" className="inputDetail" />
              </li>
              <li className="list-group-item">
                <p className="pCheckout">Código Postal:</p>
                {' '}
                <input style={{ textAlign: 'center' }} onChange={handleChange} type="text" name="cp" className="inputDetail" />
              </li>
              <li className="list-group-item">
                <p className="pCheckout">Localidad:</p>
                {' '}
                <input style={{ textAlign: 'center' }} onChange={handleChange} type="text" name="localidad" className="inputDetail" />
              </li>
            </ul>
          </div>
        </div>
              ...
              <tbody>
                ...
                {props.carrito.map(producto => (
                  <tr key={producto.id}>
                    ...
                    <td>
                      $
                      {Totales(producto.precio, producto.carrito.cantidad)}
                    </td>
                  </tr>
                ))}
                <tr style={{ borderTop: 'solid thin darkslategrey' }}>
                  <td />
                  <td />
                  <td style={{ fontWeight: '600' }}>{cantidadTotal}</td>
                  <td />
