  ...
  <div className="modal-dialog" role="document">
    <div className="modal-content">
      ...
      <div className="modal-footer">
        <button type="button" className="btn btn-primary" data-dismiss="modal" onClick={() =>handleClick(categoria.id)}>Si</button>
...
<div className="modal fade" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
 ...
 {/* <button id='eliminar'>Hover Me</button> */}
        ...
        {categorias.map((categoria) => (
          <div key={categoria.id} style={{ cursor: 'pointer' }} className="list-group">
            <button id='eliminar' data-toggle="modal" data-target="#definiteModal" type="button" value={categoria.id}
              onClick={(e) => {setCategoria(categoria = {nombre : categoria.name, id: categoria.id});
              }}
      ...
      <form id='newCategory' className='formProdAm' onSubmit={onSubmit} >
        ...
        <button style={{marginLeft: '44%'}} data-toggle="modal" data-target="#confirmModal" onSubmit={onSubmit} className='btn btn-lg btn-success btn-cat'>SUBMIT</button>
      </form >
    </div>
  )
}
